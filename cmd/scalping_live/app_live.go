...
// ScalpingLiveApp manages the scalping strategy in LIVE mode (production)
type ScalpingLiveApp struct {
...
// NewScalpingLiveApp creates a new scalping paper/live application
func NewScalpingLiveApp(config *shared.Config, mode string) *ScalpingLiveApp {
	...
	// TOUJOURS utiliser Testnet (pas de restrictions géographiques)
	binance.UseTestnet = true
	binanceClient = binance.NewClient("", "")

	return &ScalpingLiveApp{
		config:        config,
		mode:          mode,
...
func (app *ScalpingLiveApp) Run(ctx context.Context) error {
...
func (app *ScalpingLiveApp) loadInitialKlines() error {
	symbol := app.config.BinanceData.Symbols[0]
	timeframe := app.config.Strategy.ScalpingConfig.Timeframe
...
func (app *ScalpingLiveApp) runTimerLoop(ctx context.Context) error {
...
func (app *ScalpingLiveApp) processTimerTick() error {
	now := time.Now()
	fmt.Printf("[%s] 🔄 Tick...\n", now.Format("15:04:05"))
...
func (app *ScalpingLiveApp) processTickAsync() {
...
func (app *ScalpingLiveApp) fetchLatestKlines() ([]Kline, error) {
	symbol := app.config.BinanceData.Symbols[0]
	timeframe := app.config.Strategy.ScalpingConfig.Timeframe
...
func (app *ScalpingLiveApp) detectNewCompletedCandles(newKlines []Kline) []int64 {
	app.mu.Lock()
	defer app.mu.Unlock()
...
func (app *ScalpingLiveApp) processMarker(timestamp int64) error {
