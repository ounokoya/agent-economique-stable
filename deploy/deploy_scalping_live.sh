#!/bin/bash
# 🔨 Script de compilation et déploiement du binaire Scalping Live
...
REMOTE_USER="root"
REMOTE_BASE_DIR="/root/data/scalping-live"
BINARY_NAME="scalping_live"
...
echo -e "${BLUE}🔨 Compilation et Déploiement Scalping Live${NC}"
echo "================================================"
...
if [ ! -f "cmd/scalping_live/main.go" ]; then
    log_error "Code source manquant: cmd/scalping_live/main.go"
    exit 1
fi
...
if ! go build -o $BINARY_NAME ./cmd/scalping_live; then
...
echo "   ./deploy/deploy_scalping_live_nomad.sh"
echo ""
