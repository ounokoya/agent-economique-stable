#!/bin/bash
# 🔨 Script de compilation et déploiement du binaire Scalping Live Bybit
...
REMOTE_USER="root"
REMOTE_BASE_DIR="/root/data/scalping-live-bybit"
...
echo -e "${BLUE}🔨 Compilation et Déploiement Scalping Live Bybit${NC}"
echo "======================================================="
echo "• Binaire: $BINARY_NAME"
echo "• Serveur: $REMOTE_USER@$REMOTE_HOST"
echo "• Destination: $REMOTE_BASE_DIR/"
echo "• Exchange: Bybit (pas de restrictions géo)"
echo "======================================================="
...
if [ ! -f "cmd/scalping_live_bybit/main.go" ]; then
    log_error "Code source manquant: cmd/scalping_live_bybit/main.go"
    exit 1
fi
...
if ! go build -o $BINARY_NAME ./cmd/scalping_live_bybit; then
...
echo ""
echo -e "${GREEN}✅ Exchange: Bybit (pas de restrictions géographiques)${NC}"
echo ""
echo -e "${BLUE}🚀 Prochaine étape:${NC}"
echo "   ./deploy/deploy_scalping_live_bybit_nomad.sh"
echo ""
