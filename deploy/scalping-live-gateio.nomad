job "scalping-live-gateio" {
  datacenters = ["dc1"]
  type = "service"

  group "scalping-live-gateio-group" {
    count = 1
    ...
    task "setup-dirs" {
      ...
      config {
        command = "/bin/mkdir"
        args    = ["-p", "/root/data/scalping-live-gateio/config", "/root/data/scalping-live-gateio/logs", "/root/data/scalping-live-gateio/state"]
      }
    ...
    task "scalping-live-gateio-5m" {
      driver = "raw_exec"

      config {
        command = "/root/data/scalping-live-gateio/scalping_live_gateio"
      ...
      template {
        data = <<EOH
# Configuration Scalping Live Gate.io - SOLUSDT 5m
# Version: 1.0.0
# Exchange: Gate.io (pas de restrictions géographiques)

# Configuration données Binance (réutilisé pour structure)
binance_data:
  cache_root: "data/gateio"
  symbols: 
    - "SOLUSDT"
  timeframes:
    - "5m"
