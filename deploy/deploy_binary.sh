#!/bin/bash
# 🔨 Script de compilation et déploiement du binaire Agent Economique Live Notifications
...
BINARY_NAME="agent_economique_live_notifications"
...
echo -e "${BLUE}🔨 Compilation et Déploiement du Binaire Agent Economique${NC}"
echo "================================================"
...
if [ ! -f "backend/cmd/agent_economique_live_notifications/main.go" ]; then
    log_error "Code source manquant: backend/cmd/agent_economique_live_notifications/main.go"
    exit 1
fi
...
cd backend
if ! go build -o $BINARY_NAME cmd/agent_economique_live_notifications/main.go; then
...
ssh $REMOTE_USER@$REMOTE_HOST "ls -la $REMOTE_BASE_DIR/"
